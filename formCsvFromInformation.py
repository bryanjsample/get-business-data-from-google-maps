...
def formCsv(csv_write, business_information, dir_name, csv_name):
    CsvRowValues = []
    cwd = os.getcwd()
    ...
    if csv_write == 'write':
    #write csv file
        with open(f'{cwd}/searches/{dir_name}/{csv_name}.csv', 'w', newline='') as csvfile:
            ...
            for name, info_ls in business_information.items():
                ...
                CsvRowValues.clear()
            csvfile.close()
        print(f'{csv_name} written inside of {cwd}/searches/{dir_name}')
    #append csv
    if csv_write == 'append':
        with open(f'{cwd}/searches/{dir_name}/{csv_name}.csv', 'a', newline='') as csvfile:
            writer = csv.writer(csvfile, delimiter=',', quotechar='"', quoting=csv.QUOTE_ALL)
            for name, info_ls in business_information.items():
                CsvRowValues.append(name)
                for items in info_ls:
